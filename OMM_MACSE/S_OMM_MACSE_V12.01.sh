  ...
  case "$1" in
    ...
    --alignAA_soft)           ALIGN_SOFT="$2"                                || quit_pb_option ; shift 2
                              if [[ ! "$ALIGN_SOFT" =~ ^(MAFFT|MUSCLE|PRANK)$ ]]; then
                                echo " Alignment software $ALIGN_SOFT is not handled, please choose between MAFFT, MUSCLE, or PRANK"; quit_pb_option
                              fi
                              ;;
...
if [ -z ${IN_SEQ_FILE+x} ]; then echo "mandatory --in_seq_file is missing"; quit_pb_option; fi
if [ -z ${OUT_DIR+x} ]; then echo "mandatory --out_dir is missing"; quit_pb_option; fi
if [ -z ${PREFIX+x} ]; then echo "mandatory --PREFIX is missing"; quit_pb_option; fi
...
if [ $HAS_SEQ_LR -eq 1 ] ; then
  echo "" >> __all_seq.fasta
  cat "$IN_SEQ_LR_FILE">> __all_seq.fasta
fi

if [ $PRE_FILTERING -eq 1 ]; then
  echo -e "\n\n============== MACSE PRE_FILTERING\n"
  if [ $HAS_SEQ_LR -eq 1 ] ; then
    $macse -prog trimNonHomologousFragments \
      ...
      "$MIN_MEM_OPT" \
      -seq __all_seq.fasta \
      -out_NT __"${PREFIX}"_homol_tmp_NT_all.fasta \
      -out_AA __"${PREFIX}"_homol_tmp_all_AA.fasta \
      -out_trim_info __"${PREFIX}"_homol_fiter.csv \
      -out_mask_detail __"${PREFIX}"_NonHomolFilter_NT_mask_detail.fasta \
      -min_trim_in 60 \
      -min_trim_ext 45 \
      -debug
    __"${PREFIX}"_homol_tmp_NT_all.fasta
    #for s in $(grep ">" "$IN_SEQ_FILE"); do
    #  grep -A1 -e"$s$" __"${PREFIX}"_homol_tmp_NT_all.fasta
    #done > __"${PREFIX}"_homol_tmp_NT.fasta
    while IFS= read -r line ; do
      grep -A1 -e "$line$" __"${PREFIX}"_homol_tmp_NT_all.fasta
    done < <(grep ">" "$IN_SEQ_FILE") > __"${PREFIX}"_homol_tmp_NT.fasta
    #for s in $(grep ">" "$IN_SEQ_LR_FILE"); do
    #  grep -A1 -e"$s$" __"${PREFIX}"_homol_tmp_NT_all.fasta
    #done > __"${PREFIX}"_homol_tmp_NT_lr.fasta
    while IFS= read -r line ; do
      grep -A1 -e "$line$" __"${PREFIX}"_homol_tmp_NT_all.fasta
    done < <(grep ">" "$IN_SEQ_LR_FILE") > __"${PREFIX}"_homol_tmp_NT_lr.fasta
  else
    $macse -prog trimNonHomologousFragments \
      "$GC_OPT" \
      "$MIN_MEM_OPT" \
      -seq __all_seq.fasta \
      -out_NT __"${PREFIX}"_homol_tmp_NT.fasta \
      -out_AA __"${PREFIX}"_homol_tmp_AA.fasta \
      -out_trim_info __"${PREFIX}"_homol_fiter.csv \
      -out_mask_detail __"${PREFIX}"_NonHomolFilter_NT_mask_detail.fasta \
      -min_trim_in 60 \
      -min_trim_ext 45 \
  ...
  if [ "$nbKeep" -eq 0 ] ; then
    echo -e "\nError: no sequence kept after the pre-filtering.\nThis prefiltering is well adapted when your input sequence file contains many sequences (e.g > 20 sequences).\n In other cases it could be better to turn it off using the --no_prefiltering option\n" >&2
    exit 1
  fi
else
  ...
  "$LG_UTILS"/LGS_Fasta/S_fasta1L.sh \
    --in_seq_file __"${PREFIX}"_homol_tmp_NT.fasta \
    --out_seq_file __"${PREFIX}"_homol_tmp_NT_1L.fasta
  rm __"${PREFIX}"_homol_tmp_NT.fasta
  mv __"${PREFIX}"_homol_tmp_NT_1L.fasta __"${PREFIX}"_homol_tmp_NT.fasta
  ...
  #done
  while IFS= read -r line ; do
    echo "${line};NA;NA;NA;NA;NA;NA;true" >> __"${PREFIX}"_homol_fiter.csv
  done < <(grep ">" __all_seq.fasta | cut -f2 -d">")
  "${LG_UTILS}"/LGS_Fasta/S_unmask_seq.sh \
    --in_seq_file __all_seq.fasta \
    --out_seq_file __"${PREFIX}"_NonHomolFilter_NT_mask_detail.fasta
fi

if [ $FS_DETECTION -eq 1 ] ; then
  echo -e "\n\n============== MACSE FRAMESHIFT DETECTION\n"
  SEQ_LR_OPT="";
  if [ -s "__${PREFIX}_homol_tmp_NT_lr.fasta" ]; then
    SEQ_LR_OPT="-seq_lr __${PREFIX}_homol_tmp_NT_lr.fasta"
  fi
  echo -e "$macse -prog alignSequences $GC_OPT $MIN_MEM_OPT -seq __${PREFIX}_homol_tmp_NT.fasta $SEQ_LR_OPT -optim 2 -max_refine_iter 3 -local_realign_init 0.2 -out_NT __${PREFIX}_homol_tmp_NT.aln -out_AA __${PREFIX}_homol_tmp_AA.aln"
  $macse -prog alignSequences \
    "$GC_OPT" \
    "$MIN_MEM_OPT" \
    -seq __"${PREFIX}"_homol_tmp_NT.fasta "$SEQ_LR_OPT" \
    -optim 2 \
    -max_refine_iter 3 \
    -local_realign_init 0.2 \
    -out_NT __"${PREFIX}"_homol_tmp_NT.aln \
    -out_AA __"${PREFIX}"_homol_tmp_AA.aln
  ...
  $macse -prog translateNT2AA \
    "$GC_OPT" \
    -keep_final_stop_ON \
    -seq __"${PREFIX}"_homol_NT.fasta \
    -out_AA __"${PREFIX}"_homol_AA.fasta
fi
...
case $ALIGN_SOFT in
  ...
  "PRANK" )
    ...
    mv __"${PREFIX}"_homol_AA.aln.best.fas __"${PREFIX}"_homol_AA.aln
    ;;
esac
set +x
# get the NT and AA alignment with * and !
$macse -prog reportGapsAA2NT \
  -align_AA  __"${PREFIX}"_homol_AA.aln \
  -seq __"${PREFIX}"_homol_NT.fasta \
  -out_NT __"${PREFIX}"_homol_NT.aln
cp __"${PREFIX}"_homol_NT.aln __"${PREFIX}"_unmask_NT.aln
$macse -prog translateNT2AA \
  "$GC_OPT" \
  -seq __"${PREFIX}"_unmask_NT.aln \
  -out_AA __"${PREFIX}"_unmask_AA.aln \
  -keep_final_stop_ON
...
if [ $FILTERING -eq 1 ] ; then
  ...
  if [ $POST_FILTERING -eq 1 ] ; then
    $macse -prog reportMaskAA2NT \
      -mask_AA $  \
      -align_AA __"${PREFIX}"_homol_AA_Hmm${THRESHOLD}.fasta \
      -align __"${PREFIX}"_homol_NT.aln \
      -out_NT __"${PREFIX}"_final_homol_NT.aln \
      -out_mask_detail __"${PREFIX}"_hmmCleaner_mask2_detail.aln \
      -min_NT_to_keep_seq 100 \
      -mask_AA $ \
      -min_seq_to_keep_site "${MIN_SEQ_TO_KEEP_SITE}" \
      -min_percent_NT_at_ends "${MIN_PERCENT_NT_AT_ENDS}" \
      -dist_isolate_AA 3 \
    ...
    $macse -prog reportMaskAA2NT \
      ...
      -out_NT __"${PREFIX}"_final_homol_NT.aln \
      -out_mask_detail __"${PREFIX}"_hmmCleaner_mask2_detail.aln \
      -min_NT_to_keep_seq -1 \
      -mask_AA $ \
      -min_seq_to_keep_site "${MIN_SEQ_TO_KEEP_SITE}" \
      -min_percent_NT_at_ends -1 \
      -dist_isolate_AA -1 \
      -min_homology_to_keep_seq -1 \
      -min_internal_homology_to_keep_seq -1 # vérifier tous les -1 sont correctement gérés par MACSE
  fi
  printf "\n\n============== MACSE TRANSLATE\n"
  $macse -prog translateNT2AA \
    "$GC_OPT" \
    -seq __"${PREFIX}"_final_homol_NT.aln \
    -out_AA __"${PREFIX}"_final_homol_AA.aln
else
...
if [ $REPLACE_FS_GAP -eq 1 ]; then
  $macse -prog exportAlignment \
    "$GC_OPT" \
    -align __"${PREFIX}"_final_homol_NTOK.aln \
    -codonForInternalStop NNN \
    -codonForFinalStop NNN \
    -codonForInternalFS --- \
    -codonForExternalFS --- \
    -out_NT __"${PREFIX}"_final_homol_NTOK_export.aln \
    -out_AA __"${PREFIX}"_final_homol_AAOK_export.aln
else
  $macse -prog exportAlignment \
    "$GC_OPT" \
    -align __"${PREFIX}"_final_homol_NTOK.aln \
...
if [ $PRE_FILTERING -eq 1 ] || [ $FILTERING -eq 1 ] ; then
...
if [ $PRE_FILTERING -eq 1 ] && [ $FILTERING -eq 0 ] ; then
  "${LG_UTILS}"/LGS_Fasta/S_mask_removed_seq.sh \
    --in_seq_file __"${PREFIX}"_NonHomolFilter_NT_mask_detail.fasta \
    --in_keep_seq_info __"${PREFIX}"_homol_fiter.csv \
    --out_seq_file __"${PREFIX}"_NonHomolFilter_RmSeq_NT_mask_detail.fasta \
    --col_keep_info 8
...
if [ $FILTERING -eq 1 ] ; then
  "${LG_UTILS}"/LGS_Fasta/S_mask_removed_seq.sh \
    ...
    --out_seq_file __"${PREFIX}"_NonHomolFilter_RmSeq_NT_mask_detail.fasta \
    --col_keep_info 8
  $macse -prog mergeTwoMasks \
    -mask_file1 __"${PREFIX}"_NonHomolFilter_RmSeq_NT_mask_detail.fasta \
    -mask_file2 __"${PREFIX}"_hmmCleaner_mask2_detail.aln \
    -out_mask_detail __"${PREFIX}"_maskFull_detail.fasta \
    -out_trim_info __"${PREFIX}"_maskFull_stat.csv
  cp __"${PREFIX}"_homol_fiter.csv "${OUT_DIR}"/"${PREFIX}"_maskHomolog_stat.csv
